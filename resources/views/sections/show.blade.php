...
<tbody>@foreach($activities as $a)<tr class="border-t"><td class="p-2">{{ $a->activity_date->translatedFormat('d F Y') }}</td><td class="p-2"><a class="text-blue-700" href="/kegiatan/{{ $a->id }}">{{ $a->title }}</a></td><td class="p-2 text-right">{{ number_format($a->budget_pagu,0,',','.') }}</td><td class="p-2 text-center text-xs">{{ $a->status }}</td></tr>@endforeach</tbody></table></div>
