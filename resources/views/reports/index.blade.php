...
<div class="bg-white rounded shadow overflow-x-auto mb-4"><table class="w-full text-sm"><thead class="bg-slate-100"><tr><th class="p-2 text-center">No</th><th class="p-2 text-left">Kode</th><th class="p-2">Jml Kegiatan</th><th class="p-2 text-right">Kebutuhan</th><th class="p-2 text-right">Pagu</th><th class="p-2 text-right">Realisasi</th><th class="p-2 text-right">Sisa</th></tr></thead>
<tbody>@foreach($perRekening as $r)<tr class="border-t"><td class="p-2 text-center">{{ $loop->iteration }}</td><td class="p-2 font-mono">{{ $r['code'] }}</td><td class="p-2 text-center">{{ $r['count'] }}</td><td class="p-2 text-right font-semibold text-blue-700">{{ number_format($r['kebutuhan'],0,',','.') }}</td><td class="p-2 text-right font-semibold">{{ number_format($r['pagu'],0,',','.') }}</td><td class="p-2 text-right">{{ number_format($r['realisasi'],0,',','.') }}</td><td class="p-2 text-right">{{ number_format($r['pagu'] - $r['realisasi'],0,',','.') }}</td></tr>@endforeach</tbody></table></div>
<h2 class="font-semibold mb-2">Detail Kegiatan</h2>
<div class="bg-white rounded shadow overflow-x-auto"><table class="w-full text-sm"><thead class="bg-slate-100"><tr><th class="p-2 text-center">No</th><th class="p-2 text-left">Tanggal</th><th class="p-2 text-left">Judul</th><th class="p-2 text-right">Kebutuhan</th><th class="p-2 text-left">Satuan</th><th class="p-2 text-right">Pagu</th><th class="p-2 text-right">Realisasi</th><th class="p-2 text-right">Sisa</th></tr></thead>
<tbody>@foreach($activities as $a)<tr class="border-t"><td class="p-2 text-center">{{ $activities->firstItem() + $loop->index }}</td><td class="p-2">{{ $a->activity_date->translatedFormat('d F Y') }}</td><td class="p-2"><a class="text-blue-700" href="/kegiatan/{{ $a->id }}">{{ $a->title }}</a><br><span class="text-xs text-slate-500">{{ $a->section->name }}</span></td><td class="p-2 text-right font-semibold text-blue-700">{{ number_format($a->requirement_qty,0,',','.') }}</td><td class="p-2 text-xs whitespace-nowrap">{{ $a->unit }}</td><td class="p-2 text-right">{{ number_format($a->budget_pagu,0,',','.') }}</td><td class="p-2 text-right">{{ number_format($a->budget_realization,0,',','.') }}</td><td class="p-2 text-right font-semibold">{{ number_format($a->budget_pagu - $a->budget_realization,0,',','.') }}</td></tr>@endforeach</tbody></table></div>
