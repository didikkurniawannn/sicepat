...
@endif
<table><thead><tr><th>Bulan</th><th>Bidang</th><th>Kode Rekening</th><th>Judul</th><th>Kebutuhan</th><th>Satuan</th><th>Pagu</th><th>Realisasi</th><th>Sisa</th></tr></thead>
<tbody>@foreach($activities as $a)<tr @if(!empty($dupColors[$a->account_code])) style="background-color:{{ $dupColors[$a->account_code] }}" @endif><td>{{ $a->activity_date->translatedFormat('d F Y') }}</td><td>{{ $a->section->name }}</td><td>{{ $a->account_code }}</td><td>{{ $a->title }}</td><td>{{ number_format($a->requirement_qty,0,',','.') }}</td><td>{{ $a->unit }}</td><td>{{ number_format($a->budget_pagu,0,',','.') }}</td><td>{{ number_format($a->budget_realization,0,',','.') }}</td><td>{{ number_format($a->budget_pagu - $a->budget_realization,0,',','.') }}</td></tr>@endforeach</tbody></table>
</body></html>
